...
- (2) Bootstrap the `./system` layer:
    - Install ArgoCD
    - Configure the root app to manage itself and other layers

From now on, ArgoCD will do the rest:

- (3) Build remaining components in the `./system` layer (storage, networking, monitoring, ArgoCD, etc.)
- (4) Build the `./platform` layer (Gitea, Grafana, SSO, etc)
- (5) Build the `./apps` layer: user facing applications
